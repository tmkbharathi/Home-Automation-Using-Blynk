/*************************************************************
Title         :   Home automation using blynk
Description   :   To control light's brigntness with brightness,monitor
temperature , monitor water level in the tank through blynk app Pheripherals  :
Arduino UNO , Temperature system, LED, LDR module, Serial Tank, Blynk cloud,
Blynk App.
 *************************************************************/

// Template ID, Device Name and Auth Token are provided by the Blynk.Cloud
// See the Device Info tab, or Template settings
#define BLYNK_TEMPLATE_ID "TMPL3YixHkYcj"
#define BLYNK_TEMPLATE_NAME "Home automation"
#define BLYNK_AUTH_TOKEN "Ach5e5z58XEAvlNsgshrWII59pAFPnYy"

// Toggle this to enable/disable Blynk connection
// #define BLYNK_ENABLED

// Comment this out to disable prints
#ifdef BLYNK_ENABLED
#define BLYNK_PRINT Serial
#endif

#ifdef BLYNK_ENABLED
#include <BlynkSimpleEthernet.h>
#endif
#include <Ethernet.h>
#include <LiquidCrystal_I2C.h>
#include <SPI.h>
#include <Wire.h>

#include "ldr.h"
#include "main.h"
#include "serial_tank.h"
#include "temperature_system.h"

char auth[] = BLYNK_AUTH_TOKEN;
bool heater_sw, inlet_sw, outlet_sw, cooler_sw;
unsigned int tank_volume;

// Mac address should be different for each device in your LAN
byte mac[] = {0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED};

#ifdef BLYNK_ENABLED
BlynkTimer timer;
#endif

LiquidCrystal_I2C
    lcd(0x27, 16,
        2); // set the LCD address to 0x27 for a 16 chars and 2 line display
#ifdef BLYNK_ENABLED
// This function is called every time the Virtual Pin 0 state changes
/*To turn ON and OFF cooler based virtual PIN value*/
BLYNK_WRITE(COOLER_V_PIN) {
  cooler_sw = param.asInt();
  // If cooler button is ON in blynk, turn on cooler
  if (cooler_sw) {
    cooler_control(ON);
    lcd.setCursor(8, 0);
    lcd.print("C_ON");
  } else {
    cooler_control(OFF);
    lcd.setCursor(8, 0);
    lcd.print("C_OF");
  }
}
/*To turn ON and OFF heater based virtual PIN value*/
BLYNK_WRITE(HEATER_V_PIN) {
  heater_sw = param.asInt();
  // If heater button is ON in blynk, turn on heater
  if (heater_sw) {
    heater_control(ON);
    lcd.setCursor(8, 0);
    lcd.print("H_ON");
  } else {
    heater_control(OFF);
    lcd.setCursor(8, 0);
    lcd.print("H_OF");
  }
}
/*To turn ON and OFF inlet vale based virtual PIN value*/
BLYNK_WRITE(INLET_V_PIN) {
  inlet_sw = param.asInt();
  if (inlet_sw) {
    enable_inlet();
    lcd.setCursor(8, 1);
    lcd.print("I_ON");
  } else {
    disable_inlet();
    lcd.setCursor(8, 1);
    lcd.print("I_OF");
  }
}

/*To turn ON and OFF outlet value based virtual switch value*/
BLYNK_WRITE(OUTLET_V_PIN) {
  outlet_sw = param.asInt();
  if (outlet_sw) {
    enable_outlet();
    lcd.setCursor(12, 1);
    lcd.print("O_ON");
  } else {
    disable_outlet();
    lcd.setCursor(12, 1);
    lcd.print("O_OF");
  }
}

// Syncing the initial state of the hardware with the Blynk App
BLYNK_CONNECTED() {
  Blynk.virtualWrite(COOLER_V_PIN, cooler_sw);
  Blynk.virtualWrite(HEATER_V_PIN, heater_sw);
  Blynk.virtualWrite(INLET_V_PIN, inlet_sw);
  Blynk.virtualWrite(OUTLET_V_PIN, outlet_sw);
  Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "System Connected to Blynk Cloud\n");
}
#endif
#ifdef BLYNK_ENABLED
/* To display temperature and water volume as gauge on the Blynk App*/
void update_temperature_reading() {
  // You can send any value at any time.
  // Please don't send more that 10 values per second.
  Blynk.virtualWrite(TEMPERATURE_GAUGE, read_temperature());
  // Use the cached value from loop() to prevent serial port conflicts
  Blynk.virtualWrite(WATER_VOL_GAUGE, tank_volume);
}
#endif

/*To turn off the heater if the temperature raises above 35 deg C*/
void handle_temp(void) {
  float temp = read_temperature();

  // If temperature > 35, turn off heater and turn on fan (cooler)
  if (temp > 35.0) {
    if (heater_sw) {
      heater_sw = 0;
      heater_control(OFF);
      lcd.setCursor(8, 0);
      lcd.print("H_OF");
#ifdef BLYNK_ENABLED
      Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Temperature is above 35°C \n");
      Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Turning OFF the heater \n");
      Blynk.virtualWrite(HEATER_V_PIN, OFF);
#endif
    }

    if (cooler_sw == OFF) {
      cooler_sw = ON;
      cooler_control(ON);
      lcd.setCursor(8, 0);
      lcd.print("C_ON");
#ifdef BLYNK_ENABLED
      Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Turning ON the cooler \n");
      Blynk.virtualWrite(COOLER_V_PIN, ON);
#endif
    }
  }
  // Optional: Turn OFF cooler if temp falls back to safe level (e.g. < 30)
  else if (temp < 30.0) {
    if (cooler_sw == ON) {
      cooler_sw = OFF;
      cooler_control(OFF);
      lcd.setCursor(8, 0);
      lcd.print("C_OF");
#ifdef BLYNK_ENABLED
      Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Turning OFF the cooler \n");
      Blynk.virtualWrite(COOLER_V_PIN, OFF);
#endif
    }
  }
}

/*To control water volume above 2000ltrs*/
void handle_tank(void) {
  if ((tank_volume < 2000) && (inlet_sw == OFF)) {
    enable_inlet();
    lcd.setCursor(8, 1);
    lcd.print("I_ON");
    inlet_sw = ON;

    // To reflect ON on inlet virtual pin
#ifdef BLYNK_ENABLED
    Blynk.virtualWrite(INLET_V_PIN, ON);

    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN,
                       "Water volume is < 2000 litres \n");
    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Turning ON the inlet valve \n");
#endif
  }

  // Checking if tank volume is full, then turn OFF the inlet valve
  if ((tank_volume == 3000) && (inlet_sw == ON)) {
    disable_inlet();
    lcd.setCursor(8, 1);
    lcd.print("I_OF");
    inlet_sw = OFF;

    // To reflect ON on inlet virtual pin
#ifdef BLYNK_ENABLED
    Blynk.virtualWrite(INLET_V_PIN, OFF);

    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Water volume is FULL \n");
    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Turning OFF the inlet valve \n");
#endif
  }

  // Handle Outlet Local Override (Switch 1 on Pin 2)
  // LOW = Pressed/ON (using INPUT_PULLUP)
  bool sw1_state = (digitalRead(SW1_OUTLET_PIN) == LOW);
  if (sw1_state != outlet_sw) {
    outlet_sw = sw1_state;
    if (outlet_sw) {
      enable_outlet();
      lcd.setCursor(12, 1);
      lcd.print("O_ON");
    } else {
      disable_outlet();
      lcd.setCursor(12, 1);
      lcd.print("O_OF");
    }
#ifdef BLYNK_ENABLED
    Blynk.virtualWrite(OUTLET_V_PIN, outlet_sw);
#endif
  }
}

void setup(void) {
  // initialise LCD
  lcd.init();
  // turn backlight
  lcd.backlight();
  // clear clcd
  lcd.clear();
  // point cursor to home
  lcd.home();

  // initialising garden lights as output pin
  init_ldr();

  // initialising temp system
  init_temperature_system();

  init_serial_tank();

  pinMode(SW1_OUTLET_PIN, INPUT_PULLUP);

#ifdef BLYNK_ENABLED
  // For updating the timer on Blynk app for every 0.5 sec
  timer.setInterval(500L, update_temperature_reading);
#endif

  lcd.clear();

#ifdef BLYNK_ENABLED
  lcd.setCursor(0, 0);
  lcd.print("Connecting...");
#endif

#ifdef BLYNK_ENABLED
  // Specifically map W5500 Ethernet CS pin to 10 for Arduino Uno
  Ethernet.init(10);

  // Define Network Fallbacks
  IPAddress ip_eth(192, 168, 1, 100);
  IPAddress gateway(192, 168, 1, 1);
  IPAddress dns(8, 8, 8, 8); // Google DNS
  IPAddress subnet(255, 255, 255, 0);

  // Try DHCP first
  if (Ethernet.begin(mac) == 0) {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("DHCP Failed");
    delay(1000);
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Static IP...");
    // Static Backup with routing info
    Ethernet.begin(mac, ip_eth, dns, gateway, subnet);
  } else {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("DHCP Success");
  }
  delay(1000);

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Blynk init...");

  // connecting arduino to blynk server
  Blynk.config(auth, "blynk.cloud", 80);
  Blynk.connect();
#endif

  // Initializing switch states to OFF
  heater_sw = OFF;
  cooler_sw = OFF;
  inlet_sw = OFF;
  outlet_sw = OFF;

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("T=");

  // Set cursor to 2nd line
  lcd.setCursor(0, 1);
  lcd.print("V=");

  // Display initial status on LCD
  lcd.setCursor(8, 0);
  lcd.print("H_OF");
  lcd.setCursor(8, 1);
  lcd.print("I_OF");
  lcd.setCursor(12, 1);
  lcd.print("O_OF");
}

void loop(void) {
  // control brightness of garden lights using LDR sensor
  brightness_control();

  // For displaying the temperature on CLCD
  lcd.setCursor(2, 0);
  String temp = String(read_temperature(), 1);
  lcd.print(temp);
  // Pad with spaces to reach index 7
  for (int i = temp.length(); i < 5; i++) {
    lcd.print(" ");
  }

  tank_volume = volume();
  lcd.setCursor(2, 1);
  String vol = String(tank_volume);
  lcd.print(vol);
  // Pad with spaces to reach index 7
  for (int i = vol.length(); i < 5; i++) {
    lcd.print(" ");
  }

  // To check threshold temp and controlling heater
  handle_temp();

  // To monitor the volume of water, turn ON the inlet valve if V < 2000 ltrs
  handle_tank();

#ifdef BLYNK_ENABLED
  timer.run();
#endif
  // To run the Blynk application
#ifdef BLYNK_ENABLED
  Blynk.run();
#endif
  delay(10);
}
